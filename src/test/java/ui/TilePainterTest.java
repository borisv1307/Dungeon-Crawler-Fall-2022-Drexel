...
public class TilePainterTest {
	...
	public void paint_tiles() {
		...
		Mockito.when(game.getTileFromCoordinates(2, 0)).thenReturn(TileType.NEAR);
		Mockito.when(game.getTileFromCoordinates(2, 1)).thenReturn(TileType.PASSABLE);
		Mockito.when(game.getTileFromCoordinates(2, 2)).thenReturn(TileType.PASSABLE);
		...
		inOrder.verify(graphics).fillRect(20, 0, 10, 20);
		inOrder.verify(graphics).fillRect(20, 20, 10, 20);
		inOrder.verify(graphics).fillRect(20, 40, 10, 20);
	...
	public void paint_player() {
		...
		Mockito.verify(graphics).fillRect(30, 60, 10, 20);
	}
