...
public class DungeonCrawlerTest {
	...
	@Test
	public void create_initial_projectiles() {
		dungeonCrawler.run();
		Mockito.verify(projectileHandler, Mockito.times(1)).createProjectiles();
	}
	...
	public void do_not_run_if_not_enough_time_has_elapsed() {
		...
		dungeonCrawler.run();
		Mockito.verify(gameEngine, Mockito.never()).run(gameFrame, projectileHandler);
	}
	...
	public void run_and_exit_only_when_set_exit_true() throws InterruptedException {
		...
		Mockito.verify(gameEngine).run(gameFrame, projectileHandler);
